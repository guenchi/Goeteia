...
from a tagged source tree; a directory here is never rebuilt in place.
    ...
    https://cdn.goeteia.dev/<version>/web.mjs       loader: loadGoeteia, loadGoeteiaAuto, loadGoeteiaWorker, hasWasmGC
    https://cdn.goeteia.dev/<version>/jsbridge.mjs  the js.* import bridge (web.mjs imports it)
    https://cdn.goeteia.dev/<version>/worker.mjs    worker-side loader (web.mjs resolves it relative to itself)
    https://cdn.goeteia.dev/<version>/sexpr.mjs     s-expression wire codec for pages that do not run Goeteia
    https://cdn.goeteia.dev/<version>/react.mjs     embedding Goeteia components in a React tree
...
Usage, from any page:
    ...
    <script type="module">
      import { loadGoeteiaAuto } from 'https://cdn.goeteia.dev/1.5.8/web.mjs';
