...
Wasm GC is the fast path, not a shipping requirement: `--js` compiles
the same source to a plain-JavaScript ES module instead — one program,
two artifacts, identical behavior down to the errors (a program that
traps on wasm raises at the same point on JS).  Every test runs on
both.  See [The JS target](#the-js-target).
...
- `(web embed)` — the two-artifact mount section, assembled:
  `(mount-html js-text wasm-ref rt-url)` returns the fragment that
  carries the `--js` module inline in an inert
  `<script type="goeteia/js">` tag and hands the wasm reference to
  `loadGoeteiaAuto`, which picks by engine support at load time.
  `wasm-ref` is a URL — the lean shape, the module fetches and caches
  on its own — or the module's bytes, which embed as a
  `data:application/wasm;base64,` URI (`wasm->data-uri`) for a page
  that depends on nothing beside it.  `bin/goeteia-mount.mjs` is the
  CLI over it, so a site generator and the command line splice the
  same fragment

`examples/counter.html` is a page scripted entirely in Goeteia;
`examples/counter-single.html` is that page as ONE self-contained
file, generated by `examples/mk-counter-single.sh`;
...
engines without Wasm GC — see [The JS target](#the-js-target):

```
$ node rt/compile.mjs --js goeteia.wasm program.ss program.js
$ node rt/runjs.mjs program.js                  # same io hooks, same printed result
$ node bin/goeteia-mount.mjs program.ss         # the browser mount section for both artifacts
```
...
the compiler itself.  A third column compiles every test with `--js`
and holds the result to the same expectation — the two targets answer
one oracle, test by test — and twelve `test/js-backend-*.mjs` cases
pin what an expectation line cannot: the traps (division by zero,
bounds, arity, operand types, float conversion), memory growth, the
tail-call trampoline and the JSPI probes.  `bench/`
holds standalone microbenchmarks (`bench/perf.ss` covers the web
stack's per-frame hot paths).
...
`--js` forks the pipeline at the last stage only: reading, macro
expansion and every optimization pass are shared, and the JS emitter
is a sibling of the byte emitter walking the same core language.  The
value representation mirrors the wasm one (fixnums and chars stay
tagged JS numbers, flonums box, pairs are tagged object literals,
strings are `Uint8Array`s), the prelude is the same Scheme compiled
the same way, and both hosts must emit byte-identical JS text.  Output
is one self-contained ES module exporting `main(io)` -- kernel
included, nothing to import.

Behavior is target-identical *including failure*: division by zero,
out-of-bounds collection and byte-memory access, wrong operand types,
invalid float conversions and arity violations all trap where wasm
traps, and memory growth detaches old views and answers -1 on failure
exactly as `memory.grow` does -- and where `WebAssembly` is absent
altogether, the staging memory falls back to a plain `ArrayBuffer`
stand-in, so restricted embedded JS environments still run the module.
Non-self tail calls trampoline, so mutual recursion runs in constant
JS stack like `return_call`.  What the target does cost is speed, not
reach: boxed flonums, scalar SIMD.
`js-await` cannot suspend without JSPI, so the kernel hides
