...
(define gbuf (fx-target-mrt! 3 720 400))
;; the lights accumulate in real HDR; grade tonemaps (ACES) and
;; gamma-encodes, FXAA smooths the result onto the canvas
(define hdr (fx-target-hdr! 720 400))
(define hdr2 (fx-target-hdr! 720 400))  ; after reflections
(define ldr (fx-target! 720 400))
(define grade (make-grade))
(define fxaa (make-fxaa))
(define proj (m4-perspective 0.9 (/ 720.0 400.0) 0.5 80.0))
...
(fx-loop!
 (lambda (t dt)
   ...
   (let* ((a (fl* 0.1 t))
     ...
     (grade-run! grade (fx-target-texture hdr2) ldr 'aces 1.3 720 400)
     (fxaa-run! fxaa (fx-target-texture ldr) #f 720 400))))
