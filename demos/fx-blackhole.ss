  ...
  (fx-program!
   '((attribute vec3 a_dir)
     ...
     (define (main) void
       ...
       ;; the DSL's (if ...) is single-branch: EVERY trailing form
       ;; would land inside it -- this needs the explicit if-else
       (if-else (> pv.z (- (fl 0) "0.05"))
         (;; behind or grazing the camera: clip it away
          (set! gl_Position (vec4 (fl 0) (fl 0) "3.0" (fl 1)))
          (set! gl_PointSize (fl 1)))
...
(fx-loop!
 (lambda (t dt)
   (let* ((a (fl* 0.045 t))
     ...
     (spass! 1.0 0.99 0.16 0.60)
     (spass! 1.0 1.31 0.07 0.40)
     (cmd-blend! #f))))
