...
**The unit/decimal forms take the fraction's digits AS WRITTEN**
(Goeteia >= 1.6.0, the same rule as `fl`): `(rem 1 5)` is `1.5rem`,
`(em 0 92)` -> `0.92em`, `(px 13 5)` -> `13.5px`, `(dec 0 8)` -> `0.8`.
Trailing zeros are dropped (`(rem 1 50)` is still `1.5rem`). A leading
zero is not a padding hint -- Scheme reads `05` as `5` -- so a value
with leading zeros after the point takes the width as a THIRD operand:
`0.05em` is `(em 0 5 2)`. (Before 1.6.0 the second operand was
hundredths and `(rem 1 5)` meant `1.05rem`; do not copy that from old
sites.) Negative lengths below one (`-0.02em`) have no numeric form --
`-0` reads as `0` -- write them as strings; gradients, transforms, `calc()`, font stacks and data
