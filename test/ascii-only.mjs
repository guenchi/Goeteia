...
//
// WHAT IT DOES NOT COVER, said rather than implied.  It reads
...
//
// IT READS UNTRACKED FILES TOO, and that was a defect for the first
...
// Arrows, stars, warning signs and the rest of the pictographs.  They
// are not a language barrier, so they are a separate rule from the one
// above, with a separate reason: a comment in this tree is prose, and
// prose that leans on a symbol to carry emphasis stops being readable
// where the symbol does not render -- a terminal, a diff, a patch mail,
// a plain-text export.
//
// This was not a standing problem until it suddenly was.  On 2026-09-09
// four sessions took forty-two of these into library comments in one
// night, all of them in one direction: the dispatch messages that asked
// for the work were written in that style, and the work came back
// written the same way.  Nobody introduced a rule; a habit propagated
// down a channel that had no check on it.
const MARKS = /[\u2190-\u2BFF\u{1F300}-\u{1FAFF}\uFE0F]/u;
...
test('every comment is English prose, with no pictographs carrying its emphasis', () => {
    const bad = [];
    ...
    assert.deepEqual(bad, [],
        'a comment here is English prose and carries its emphasis in words:\n  ' +
        bad.join('\n  '));
});
...
test('CONTROL the check sees a comment that has it, and not a string that does', () => {
    ...
    assert.ok(!HAN.test(';; an ordinary comment, with no marks at all'));
    // and the second rule, with its own control
    assert.ok(MARKS.test(';; a comment with \u26A0\uFE0F in it'));
    assert.ok(!MARKS.test(';; an ordinary comment -- an em dash and a colon: fine'));
...
test('CONTROL the file list and the extension map both reach the tree', () => {
    // Either one coming up empty would make the first test pass for
    // the wrong reason.
    const files = tracked();
