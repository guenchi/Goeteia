;; expect: #t
;; (gfx sprite) against recording mocks: the atlas rasterizes each
...
(import (rnrs) (web js) (gfx gl) (gfx glsl) (gfx fx) (web typeset) (gfx sprite))
...
;; u_resolution is the viewport in force when the batch is drawn, not
;; the canvas: pixel-space rects drawn under a 320x150 viewport are laid
;; out against 320x150.  (Read from GL at replay; this mock's
;; getParameter answers the last viewport it was given.)
(define viewport-res-ok
  (let ((base-v (log-len)))
    (cmd-begin!)
    (cmd-viewport! 0 0 320 150)
    (batch-begin! bt)
    (rect! bt 10 20 30 40 1 0 0 1)
    (batch-draw! bt)
    (cmd-flush!)
    (let scan ((i base-v))
      (and (< i (log-len))
...
(and init-ok meas-ok prep-ok battr-ok rect-ok once-ok text-ok lines-ok
     grow-ok grow-draw-ok load-ok sheet-ok sheet-draw-ok viewport-res-ok)
