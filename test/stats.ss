...
;; ---- GPU time: replays wrap in a query, the result reads back ----
;; the mock answers 8.3e6 ns immediately; pump past the re-typeset
;; cooldown so the HUD text re-sets with the "GPU" segment in it
(let pump ((k 0))
  (when (< k 20)
    (cmd-begin!)
    (stats-draw! hud 0.016)
    ...
    (pump (+ k 1))))
(define gpu-ok
  (and (>= (count-log "beginQuery") 1)
       (>= (count-log "endQuery") 1)
       (fl<? 8.2 (gl-gpu-ms))
       (fl<? (gl-gpu-ms) 8.4)
       ;; the typeset text rasterized a 'G' for the GPU segment
       (let* ((log2 (js-get (js-global) "__2dlog"))
              (n (js->number (js-get log2 "length"))))
         (let loop ((i 0))
           (and (< i n)
                (let ((e (js->string (js-index log2 i))))
                  (or (and (>= (string-length e) 10)
                           (string=? (substring e 0 10) "fillText:G"))
                      (loop (+ i 1)))))))))

(and count-ok hud-ok frame2-ok gpu-ok)
