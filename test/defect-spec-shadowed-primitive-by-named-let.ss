;; expect: 5
;; Shadowed-primitive family, named let / %loop shape.  fl+ is shadowed by a
;; named let / %loop binding, so in operator position it is that binding, a lambda
...
(display (let loop ((fl+ (lambda (x y) 5))) (zq (fl+ 1.0 2.0) 0.0)))
