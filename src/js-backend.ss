...
(define (jx-callcc e env lctx)
  (let* ((t (jfresh!))
    ...
    (list "(()=>{const " t "=new Pair(NIL,NIL)," w "="
          (jvar-name (cdr wv))
...
(define (jp op args trees)
  ...
  (case op
    ...
    ((cons) (list "(new Pair(" (a 0) "," (a 1) "))"))
    ((car) (list "(" (a 0) ".a)"))
    ((cdr) (list "(" (a 0) ".d)"))
    ((set-car!) (list "((" (a 0) ".a=" (a 1) "),VOID)"))
    ((set-cdr!) (list "((" (a 0) ".d=" (a 1) "),VOID)"))
    ((pair?) (list "((" (a 0) " instanceof Pair)?TRUE:FALSE)"))
...
(define $js-kernel
  '("\"use strict\";"
    ...
    "class Pair{constructor(a,d){this.a=a;this.d=d;}}"
    ...
    "for(let i=xs.length-1;i>=0;i--)l=new Pair(xs[i],l);return l;};"
    ...
    "const THR=(t,v)=>{throw new Esc(new Pair(t,v));};"
