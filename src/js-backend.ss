...
(define $js-kernel
  '("\"use strict\";"
    ...
    ;; f32x4 kernels: snapshot all source lanes before the first store,
    ;; matching v128.load when dst partially overlaps a source.
    "const FV=(p)=>[MEMV.getFloat32(p,true),MEMV.getFloat32(p+4,true),"
    "MEMV.getFloat32(p+8,true),MEMV.getFloat32(p+12,true)];"
    "const F4=(op,d,p,q)=>{const x=FV(p),y=FV(q);"
    "for(let i=0;i<4;i++)MEMV.setFloat32(d+i*4,"
    "op==='+'?x[i]+y[i]:op==='-'?x[i]-y[i]:x[i]*y[i],true);};"
    "const F4SC=(d,p,s)=>{const sf=Math.fround(s),x=FV(p);"
    "for(let i=0;i<4;i++)MEMV.setFloat32(d+i*4,x[i]*sf,true);};"
    "const F4AX=(d,p,q,s)=>{const sf=Math.fround(s),x=FV(p),y=FV(q);"
    "for(let i=0;i<4;i++)MEMV.setFloat32(d+i*4,"
