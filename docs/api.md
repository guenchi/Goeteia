...
- `grade-run!` — grades a source texture into a target under a mode and an exposure, at the given size. The source is linear and the result sRGB-encoded, with `(gfx srgb)`'s curve
