...
- `mesh-lit-fs` — the matching fragment forms: `u_light` (unit vector TOWARD the light), `u_color`, `u_ambient`. `u_color` is sRGB-encoded -- the color as a picker or an 8-bit image gives it. The shader decodes it with `(gfx srgb)`'s exact curve, lights it in linear, and writes the result sRGB-encoded
- `mesh-tex-vs` — vertex forms for the textured layout, adding `a_uv` to the lit pair
- `mesh-tex-fs` — the matching fragment forms, sampling `u_tex` and shading it with the same directional light. The texture is sRGB-encoded like `u_color`, which tints it; the sample and the tint are each decoded and then multiplied, because decoding a product of encoded values is not the product of their linear values
- `mesh-normal-vs` — vertex forms for normal mapping, adding `a_tangent` and building the tangent frame
- `mesh-normal-fs` — the matching fragment forms, perturbing the normal by `u_nmap` before lighting. `u_color` is sRGB-encoded as in `mesh-lit-fs`; the normal map is data and is not decoded
- `mesh-pbr-vs` — vertex forms for the metallic-roughness model
- `mesh-pbr-fs` — the matching fragment forms: GGX specular with `u_metallic` and `u_roughness`, and an image-based ambient term when the IBL maps are bound. `u_albedo` is sRGB-encoded like `u_color`, and the probe is read as the sRGB-encoded bytes `ibl-prefilter!` writes; both are decoded here, and the tonemapped result is written sRGB-encoded
...
- `grade-run!` — grades a source texture into a target under a mode and an exposure, at the given size. The source is linear and the result sRGB-encoded, with `(gfx srgb)`'s curve. The modes are `'none`, `'reinhard`, `'aces` (Narkowicz's fit) and `'aces-hill` (Hill's fit); any other is an error. The two ACES modes are different fits and differ in brightness at one exposure -- see `tonemap-shader-functions`
- `tonemap-shader-functions` — the tone curves the grade pass uses, as shader forms for a caller to splice after its precision statement: `tonemap_reinhard`, `tonemap_aces` (Narkowicz's single-curve fit, per channel), `tonemap_aces_hill` (Stephen Hill's fit of the ACES reference rendering and output transforms: sRGB to AP1 with the RRT's saturation, the rational curve, back to sRGB, then a clamp to [0, 1]), each `vec3` to `vec3`, and `tonemap(vec3 x, float mode)`, which picks one by interval: below 0.5 none, then 1, 2 and 3 in that order. Worked out in double precision, mid-grey 0.18 comes out 0.267 under Narkowicz's fit and 0.106 under Hill's; Hill's output matrix can take a saturated colour past 1 before the clamp
...
- `surface-shader-functions` — `tangent_frame`, `apply_normal_map` and `surface_normal` as shader forms, for a caller to splice into its own shader. The tangent basis is derived from the screen-space derivatives of world position and texture coordinate, so a mesh does not have to carry a tangent attribute -- which matters for glTF, where that accessor is optional and often absent, and where its absence otherwise means the normal map cannot be used at all. `tangent_frame` answers a `mat3` whose third column is the shading normal, flipping it for a back face because the interpolated normal is the front face's and a two-sided material is otherwise lit from behind; where the texture coordinate does not change across the pixel both tangent columns are zero and the reciprocal square root of that would be infinite, so the larger squared length is floored at 1e-5; the columns stay zero, and a normal map applied through the frame leaves the shading normal untilted rather than turning it into a NaN. A triangle with zero area in UV space whose coordinate still changes along one direction gives two parallel columns instead: finite, with a tilt along that direction only. `apply_normal_map` takes the texel AS SAMPLED, channels in [0,1], and decodes it here, because a caller who has to remember the decode is one who will one day not; the scale weights the two tangent components only. `surface_normal` is written as the composition of the two, not as its own arithmetic, so the preset cannot drift from the primitives it stands for. Two things a caller owns: sampling (it has the sampler, the coordinate and any level-of-detail bias), and splicing `mat-shader-functions` ahead of these, since they call `safe_unit` and GLSL has no imports, and nothing else: `triplanar_srgb` is in `surface-srgb-shader-functions` so that this list does not require `(gfx srgb)`. `dFdx` and `dFdy` are core in ES 3.00 and need `GL_OES_standard_derivatives` enabled in ES 1.00. The triplanar functions call `texture`, the ES 3.00 name. The rest of the list, one line each:
  - `triplanar_color(sampler2D tex, vec3 point, vec3 weights) -> vec3` — three samples of one texture along the cardinal planes, blended by the caller's weights, as stored -- for a texture that holds data or linear values
  - `triplanar_normal(vec3 base, vec3 x_map, vec3 y_map, vec3 z_map, vec3 weights) -> vec3` — three normal-map texels, as sampled, turned into height gradients, projected onto the surface and applied to `base`; each map's z is floored so a flat map gives a finite gradient
  - `surface_tangent(vec3 point, vec2 uv, vec3 normal) -> vec3` — a unit tangent along the texture's V axis, from screen-space derivatives; where the UV derivatives carry no direction it substitutes an arbitrary perpendicular rather than normalizing zero
  - `skin_diffuse(vec3 normal, vec3 light, vec3 albedo) -> vec3` — a wrapped diffuse lobe, normalized, standing in for light that enters and leaves nearby without a blur pass
  - `fiber_specular(vec3 tangent, vec3 half_direction, float roughness) -> float` — an anisotropic highlight around a fiber's tangent, normalized so its energy stays roughly constant as roughness moves the exponent
  - `cloth_sheen(vec3 normal, vec3 view, vec3 light) -> float` — a sheen that grows as the view grazes the surface: `(1 - max(N.V, 0))^4`, times `max(N.L, 0)`, times 0.12. It does not depend on where the light is relative to the view
  - `leaf_transmission(vec3 albedo, vec3 view, vec3 light, vec3 normal, float thickness) -> vec3` — light through a thin slab from behind: a forward lobe attenuated by thickness and tinted toward the wavelengths a leaf transmits
  - `filtered_roughness(vec3 normal, float roughness) -> float` — roughness widened by the normal's variation inside one pixel, so that a minified normal map aliases less into specular sparkle. The variation is read from screen-space derivatives, so variation finer than one pixel goes unseen: `sqrt(roughness^2 + v)`, where `v` is a quarter of the normal's squared screen-space change, held at 0.18 at most, and the result clamped to [0.16, 0.98]
  - `surface_height_blend(float coverage, float base_height, float layer_height, float width) -> float` — where a layer wins against the surface under it: its coverage biased by the height difference, with `width` as the transition
  - `water_transmission(vec3 bottom, vec3 scattering, vec3 absorption, float distance) -> vec3` — Beer-Lambert through `distance` of water, with in-scattering taking over what absorption removes
  - `shore_wetness(float ground_height, float water_height, float band) -> float` — how wet ground is from how far it sits above the water; the band is floored so zero does not make the transition a step
  - `ambient_diffuse(vec3 albedo, float metallic, vec3 normal, vec3 ground, vec3 sky) -> vec3` — hemisphere ambient, ground below and sky above, with metals taking none of it as diffuse
  - `water_path_length(vec3 eye, vec3 bottom, float height) -> float` — the submerged length of one ray from the eye to the bottom, taken along that single ray; a horizontal ray is answered by its own branch
  - `water_foam_hash(vec2 p) -> float` — a hash of position, and the base of the noise below
  - `water_foam_noise(vec2 p) -> float` — value noise built on that hash; both are functions of position alone, so foam does not swim when the camera moves
  - `water_shore_foam(vec2 position, float depth, float footprint, float time, float band) -> float` — foam at the contact line: in across the shallows and out before deep water, broken up by world-space noise so the contour has no repeating direction
- `surface-srgb-shader-functions` — `triplanar_srgb(sampler2D tex, vec3 point, vec3 weights) -> vec3` as shader forms: `triplanar_color` for a texture that holds sRGB-encoded color, decoding each of the three samples before blending them and answering linear; a texture that holds data -- a normal map, roughness, a mask -- goes through `triplanar_color`. It calls `decode_srgb`, and a compiler checks every function body in a shader whether or not anything calls it, so it is kept out of `surface-shader-functions`, which would otherwise require `(gfx srgb)` of every caller. Splice in this order: `mat-shader-functions`, `srgb-shader-functions`, `surface-shader-functions`, this list
