...
**New at this layer.** This program IS the browser half: the page is
already open, and everything it shows it BUILDS into `<div id="app">`
with `create-element` / `set-text!` / `set-attribute!` / `set-style!` /
`append-child!`. `set-inner-html!` is not a shortcut for that — it hands
the host one string, and a headless run stores that string without
parsing a single element out of it. Styling is `(web css)`: a
