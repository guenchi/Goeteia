# T3 — meshes in three dimensions

**New at this layer.** `fx-init!` takes the canvas the page already
has — any canvas element, whatever id the host gave it — and from there
owns staging memory and every resource slot: allocate through
`fx-alloc!` / `fx-buffer!` / `fx-program!`, never with hand-numbered
`gl-*!` calls, or the two numbering schemes collide. Shaders are
`(gfx glsl)` *forms*, so `fx-program!` reads their attribute and
uniform declarations back out and does the bookkeeping — locations,
interleaved offsets, uniform slots, VAOs. `(gfx mesh)` generators
produce exactly the 24-byte position+normal interleave `mesh-lit-vs`
declares, and **every size they take is a full extent, never a half**;
`fx-mesh!` stages one and uploads it lazily inside the first frame.
`fx-loop!` frames the callback with begin/viewport/flush — **one**
bridge call per frame however much it drew — and hands it `t` and `dt`
in seconds. A mat4 is a column-major 16-vector, and `(m4-mul a b)`
applies `b` FIRST: `a` is the parent frame, `b` the child's own
transform.
