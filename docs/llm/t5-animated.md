# T5 — an articulated, animated scene

**New at this layer.** A rig is a chain of frames, not a special
feature. `(m4-mul parent child)` applies the child FIRST, so a joint's
frame is "where my parent put me" times "how I turn", and a limb whose
geometry is centred on the origin translates half its own length up so
that its frame sits at the joint instead of in its middle. Interaction
and rendering live in one program: the control writes plain mutable
state, the `fx-loop!` callback reads it, and two sliders entering the
chain at different depths move the picture in two different ways —
which is what "several degrees of freedom, each separately
controllable" actually asks for. Animation and input compose: a world
transform built from `t` turns the whole rig while the joints stay
...
attributes to `gprim-layout` by name AND width. Measure a rig rather
than squinting at it: identity at bind, moving over a cycle.
