# T4 — a fragment shader

**New at this layer.** `glsl->string` is a pure function from a form
list to GLSL — the `(web css)` of shaders — so shaders compose with
`append` and `map` and verify headlessly, and the same forms render to
WGSL through `(gfx wgsl)`. `(fl W F)` is a float literal whose
fraction is the digits as written (`(fl 0 5)` → `0.5`, `(fl 0 625)` →
`0.625`); a third argument gives a minimum width, so `(fl 0 5 2)` →
`0.05`;
anything needing more places is written as a string (`"0.4545"`), and
there is no exponent syntax anywhere. A shader function of your own is
`(define (name (T arg) ...) RET stmt ...)`, with `return` as a
statement. **Every name a shader introduces** — locals, uniforms,
parameters, function names, a `for` index — is checked against the
union of the GLSL ES 1.00 and 3.00 reserved lists at generation time,
so a local called `out` or `sample` is a named error here instead of a
blank canvas whose only complaint lives in a browser console you cannot
see; built-ins are *called*, never declared, so `sin`, `mix`, `clamp`
and `length` pass straight through. `fx-fullscreen!` pairs the forms
with a built-in vertex shader and a static 4-vertex triangle strip;
`u_time` and `u_resolution` wire themselves *iff* the fragment declares
them, and anything else is an ordinary `fx-uniform!` on
`(fx-quad-program q)`.
